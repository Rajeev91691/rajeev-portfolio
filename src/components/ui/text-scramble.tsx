    ...
    >
      <span className="relative font-mono tracking-widest uppercase">
        {displayText.split("").map((char, i) => (
          <span
            key={i}
            className={`inline-block transition-all duration-150 ${isScrambling && char !== text[i] ? "text-black scale-110" : "text-foreground"
              }`}
            style={{
      ...
      <span
        className={`absolute -inset-4 rounded-lg bg-accent/5 transition-opacity duration-300 -z-10 ${isHovering ? "opacity-100" : "opacity-0"
          }`}
      />
    </div>
  )
